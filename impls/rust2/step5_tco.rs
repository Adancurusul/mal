        ...
        if $ast.len() != 3 && $ast.len() != 4 {
            ...
            match eval(&$ast[1], $env)? {
                MalType::Bool(false) | MalType::Nil => {
                    if $ast.len() == 4 {
                        Ok(MalType::TcoForm(Box::new($ast[3].clone()), $env.clone()))
                    } else {
                        Ok(mal!(nil))
                    }
                }
                _ => Ok(MalType::TcoForm(Box::new($ast[2].clone()), $env.clone())),
            }
        }
    }};
}
...
fn apply_function(f: &MalType, args: &[MalType], env: &Rc<RefCell<Env>>) -> Result<MalType, String> {
    match f {
        MalType::Function { params, body, env: fn_env, .. } => {
            ...
            Ok(MalType::TcoForm(Box::new(*body.clone()), new_env))
        }
        MalType::Symbol(s) => {
            match s.as_str() {
                ...
                _ => {
                    if let Some(val) = env.borrow().get(s) {
                        match val {
                            MalType::Function { .. } => apply_function(&val, args, env),
...
fn eval(ast: &MalType, env: &Rc<RefCell<Env>>) -> Result<MalType, String> {
    ...
    loop {
        ...
        let result = match ast {
            MalType::List(ref items) if !items.is_empty() => {
                let first = &items[0];
                match first {
                    MalType::Symbol(sym) => {
                        match sym.as_str() {
        ...
        match result {
            Ok(MalType::TcoForm(new_ast, new_env)) => {
                ast = *new_ast;
                env = new_env;
                continue;
            }
...
fn main() {
    ...
    println!("Mal (Make-A-Lisp) Step 5: TCO");
